      ...
      <div class="card-body">
        <form novalidate @submit.prevent>
          ...
          <div class="form-group">
            ...
            <div>
              <select class="form-control" v-model="typeform.outputFormat" required>
                <option value="" disabled selected>Select Output Format</option>
                <option :value="outputFormatData.transtype">{{ outputFormatData.transtype }}</option>
              </select>
            </div>
          </div>
          <div class="form-group mb-0">
            <div>
              ...
              <button type="reset" class="btn btn-primary m-l-5 ml-1">
                Cancel
              </button>
            </div>
          </div>
  ...
  methods: {
    ...
    githubCommit() {
      Swal.fire({
        ...
        if (commitMsg !== undefined) {
          ...
          console.log("commitProjectObj", commitProjectObj);
          this.$store.getters.client
    ...
    async getoutputFormat() {
      await this.$store.getters.client
        .get(`/plugins`)
        .then((res) => {
          this.outputFormatData = res.data[0];
          console.log("output format", this.selectOutputFormat);
        })
        .catch((err) => {
          console.log(err);
        });
